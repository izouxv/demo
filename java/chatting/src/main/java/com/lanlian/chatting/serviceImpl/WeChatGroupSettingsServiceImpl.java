...
@Service("weChatGroupSettingsServiceImpl")
public class WeChatGroupSettingsServiceImpl implements WeChatGroupSettingsService {
	...
	public BundlingGroupVo bundling(InviteBo inviteBo) throws Parameter_Exception, UnsupportedEncodingException {
		...
		DadaGroupPo dadaGroupPo2 = weiChatGroupSettingsDao.findGroupInfoByinviteCode(dadaGroupPo);
...
//		}
		weiChatGroupSettingsDao.bundlingGP(liveChatGidUid);
	...
	public List<MessageInfoPage> getMessage(MessagePageBo messagePageBo, String source) throws Parameter_Exception {
		...
		if (endid == 0 && startid == 0) {
			startid = weiChatGroupSettingsDao.findAllMsgCount() - messagePageBo.getCount();
		...
		if (startid == 0) {
			// 向上查询历史
			List<MessageInfoPage> list = weiChatGroupSettingsDao.findMessageInfoUp(messagePageBo);
			findUserInfoByUidToAccount(list, source);
			return list;
		}
		if (endid == 0) {
			// 向下查询，新消息
			List<MessageInfoPage> list = weiChatGroupSettingsDao.findMessageInfoDown(messagePageBo);
			findUserInfoByUidToAccount(list, source);
			return list;
		}
		return null;
...
//			System.out.println("messageInfoPage.getUid():"+messageInfoPage.getUid());
