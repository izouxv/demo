 ...
 */
@Controller
@RequestMapping(value = RequestSetting.GROUP_WECHAT_PARENT)
public class WeChatController extends MyAbstractController {

	@Resource
	@Qualifier("weChatGroupSettingsServiceImpl")
	WeChatGroupSettingsService weChatGroupSettingsServiceImpl;
	...
	public String subscribe(@RequestParam(value = "inviteCode", required = true) String inviteCode,
		...
		BundlingGroupVo bundling = weChatGroupSettingsServiceImpl.bundling(inviteBo);
	...
	public String getMessage(@RequestParam(value = "gid", required = true) int gid,
		...
		List<MessageInfoPage> messages = weChatGroupSettingsServiceImpl.getMessage(messagePageBo, ssoPo.getSource());
