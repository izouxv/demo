...
func outgoing(c *client) {
	...
	for {
		...
		case pub := <-c.obound:
			...
			DEBUG.Println(NET, "obound wrote msg, id32:", msg.MessageID)
		case msg := <-c.oboundP:
			switch msg.p.(type) {
	...
	for {
		...
		case msg := <-c.ibound:
			...
			case *packets.SubackPacket:
				DEBUG.Println(NET, "received suback, id32:", m.MessageID)
			...
			case *packets.UnsubackPacket:
				DEBUG.Println(NET, "received unsuback, id32:", m.MessageID)
			...
			case *packets.PubackPacket:
				DEBUG.Println(NET, "received puback, id32:", m.MessageID)
				// c.receipts.get(msg.MsgId()) <- Receipt{}
				// c.receipts.end(msg.MsgId())
				c.getToken(m.MessageID).flowComplete()
				c.freeID(m.MessageID)
			case *packets.PubrecPacket:
				DEBUG.Println(NET, "received pubrec, id32:", m.MessageID)
			...
			case *packets.PubrelPacket:
				DEBUG.Println(NET, "received pubrel, id32:", m.MessageID)
			...
			case *packets.PubcompPacket:
				DEBUG.Println(NET, "received pubcomp, id32:", m.MessageID)
				c.getToken(m.MessageID).flowComplete()
				c.freeID(m.MessageID)
			}
		case <-c.stop:
