...
// PrimaryField return scope's main primary field, if defined more that one primary fields, will return the one having column name `id32` or the first one
func (scope *Scope) PrimaryField() *Field {
	if primaryFields := scope.GetModelStruct().PrimaryFields; len(primaryFields) > 0 {
		if len(primaryFields) > 1 {
			if field, ok := scope.FieldByName("id32"); ok {
				return field
			}
		}
...
func (scope *Scope) buildWhereCondition(clause map[string]interface{}) (str string) {
	...
	for _, arg := range args {
		...
		case reflect.Slice: // For where("id32 in (?)", []int64{1,2})
