...
func (rp *DataPacket) SetCsrcList(csrc []uint32) {
    ...
    // For this method: context is any data after an existing (or new) CSRC list. This
    // includes RTP extension data and payload.
    offsetOld := int(rp.CsrcCount()*4 + rtpHeaderLength) // offset to old context
    offsetNew := len(csrc)*4 + rtpHeaderLength           // offset to new context
    ...
    copy(newBuf[offsetNew:], rp.buffer[offsetOld:rp.inUse]) // copy over old context
...
func (rp *DataPacket) SetExtension(ext []byte) {
    ...
    // For this method: context is any data after an existing (or new) Extension area. This
    // is the payload.
    ...
    offsetNew := offsetExt + l // offset to new context
...
func (rp *DataPacket) SetPayload(payload []byte) {
    ...
    if rp.Padding() {
        ...
        // Reduce length of inUse by length of old context, thus remove old context
